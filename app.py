...
def display_output():
    ...
    w = {
        ...
        'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.5654.219 Safari/537.36',
    }
    ...
    response1 = session.get('https://api.prizepicks.com/projections', proxy="http://4W4W4W:4W4W4W@186.179.58.62:20000", headers=w)
    print(response1.text)
    prizepicks = response1.json()
    underdog_req = session.get("https://api.underdogfantasy.com/beta/v3/over_under_lines", proxy="http://4W4W4W:4W4W4W@186.179.58.62:20000", headers=w).json()
    ...
    for appearances in underdog["over_under_lines"]:
        ...
        if UDdisplay_stat == 'Headshots on Maps 1+2' and underdog_sport == 'CS:GO':
            csgohs = UDdisplay_stat = 'CSGO MAPS 1-2 HS'
        ...
        if UDdisplay_stat == 'Kills on Map 1+2' and underdog_sport == 'Val:':
            DotaPP = UDdisplay_stat = 'VAL MAPS 1-2 KILLS'
        if UDdisplay_stat == 'CSGO MAPS 1-2 KILLS' or UDdisplay_stat == 'CSGO MAPS 1-2 HS' or UDdisplay_stat == 'LOL MAP 1 KILLS' or UDdisplay_stat == 'COD MAP 3 KILLS' or UDdisplay_stat == 'COD MAPS 1-3 KILLS' or UDdisplay_stat == 'COD MAP 1 KILLS' or UDdisplay_stat == 'VAL MAPS 1-3 KILLS' or UDdisplay_stat =='VAL MAPS 1-2 KILLS' or UDdisplay_stat =='LOL MAP 1-2 KILLS':
    ...
    for included in prizepicks['included']:
        ...
        for data1 in prizepicks['data']:
            ...
            if PPprop_type == 'MAPS 1-2 Headshots' and PPname_id == PPid and Sport_type == 'CSGO':
                ppcsheadshots = PPprop_type = 'CSGO MAPS 1-2 HS'
    ...
    for udn in udlist:
        for ppn in pplist:
            if udn["Name"] == ppn["Name"] and udn['Stat'] == ppn['Stat']:
                ...
                if final['Dif'] >=0.5 or final['Dif'] <=-0.5:
